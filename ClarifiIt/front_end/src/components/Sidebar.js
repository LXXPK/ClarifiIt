            ...
            <div className='w-full '>
                ...
                <div className='h-[calc(100vh-65px)] overflow-x-hidden overflow-y-auto scrollbar  '>
                    ...
                    {allUser.map((conv, index) => {
                        const moodClass = moodColors[conv?.userDetails?.mood] || moodColors.neutral; // Default to neutral if mood is not found

                        return (
                            <NavLink to={"/" + conv?.userDetails?._id} key={conv?._id} className={`flex items-center gap-2 py-3 px-2 border border-transparent hover:border-primary rounded cursor-pointer ${moodClass}`}>
                                <div>
                                    <Avatar
